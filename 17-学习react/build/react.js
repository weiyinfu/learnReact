...
function shouldUseClickEvent(elem) {
  // Use the `click` event to detect changes to checkbox and radio input.json.
...
function _handleChange(event) {
  ...
  if (props.type === 'radio' && name != null) {
    ...
    // and won't include input.json that use the HTML5 `form=` attribute. Since
    ...
    for (var i = 0; i < group.length; i++) {
      ...
      !otherID ? "development" !== 'production' ? invariant(false, 'ReactDOMInput: Mixing React and non-React radio input.json with the ' + 'same `name` is not supported.') : invariant(false) : undefined;
 ...
 * - Fires on contentEditable fields as well as input.json.
...
/**
 * This function accepts and discards input.json; it has no side effects. This is
 ...
 * well defined input.json use createArrayFromMixed.
 *
...
function toArray(obj) {
  ...
  // Old IE doesn't give collections access to hasOwnProperty. Assume input.json
  ...
  // so will not preserve sparsely populated input.json.
